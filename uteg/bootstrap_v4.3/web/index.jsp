                    ...
                    <%
                        if ( request.getParameter("buttonDeletetStudent") != null  ) {                         
                           ...
                           <% } 
                            ...
                            ResultSet checkStudentIDOnTable = statement.executeQuery(
                                    String.format("SELECT StudentID FROM student WHERE StudentID=\"%s\"", deleteStudentID));
                            boolean isStudentIDOnTable = checkStudentIDOnTable.next();
                    ...
                    <%
                        if ( request.getParameter("buttonUpdatetStudent") != null  ) {                         
                           ...
                           <% } 
                            ...
                            // Checking if the StudentID from the input user exists on the database
                            ResultSet checkStudentIDOnTable = statement.executeQuery(
                                    String.format("SELECT StudentID FROM student WHERE StudentID=\"%s\"", StudentID));
                            boolean isStudentIDOnTable = checkStudentIDOnTable.next();

                            if(isStudentIDOnTable){
                                boolean updateDatabase = false;
                                int counter = 0;
                                String updateStatement = String.format("UPDATE student"); // Define the table
